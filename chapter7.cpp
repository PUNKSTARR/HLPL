...
const string declkey = "#"; //chapter 7, drill 10
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			else if(s == quitkey) return Token(quit); //chapter 7, drill 11
    			else if(s == square) return Token('s'); //chapter 7, drill 7
    			else if(s == power) return Token ('p'); //chapter 7, drill 9
...
double primary()
{
	...
	switch (t.kind)
	{
		...
		case '+':
			return primary();
		case 's': //chapter 7, drill 7
		{
			char ch1;
			double CanBeSquared;
			if(cin>>ch1 && ch1 == '('){
				CanBeSquared =expression();
			}
			else error("The function starts with (!"); //chapter 7, drill 8
			if(ts.get().kind == ')'){
			
				if(CanBeSquared >=0)
					return sqrt(CanBeSquared);
				else error(" argument cannot be smaller than 0!"); //Chapter 7, drill 8
				}
			else error(" ) is needed for the end of the function!"); //Chapter 7, drill 8
		}
		case 'p':
		{
			char ch1;
